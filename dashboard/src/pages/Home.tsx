...
import { getAuroraMonitor, AuroraMetrics } from '../services/auroraMonitor';
...
export default function Home() {
  ...
  const [metrics, setMetrics] = useState<AuroraMetrics | null>(null);
  const [cpuHistory, setCpuHistory] = useState<number[]>([30, 35, 32, 38, 35, 40, 38, 42, 40, 45, 43, 48, 45]);
  const [memHistory, setMemHistory] = useState<number[]>([50, 52, 51, 54, 53, 56, 55, 58, 57, 60, 59, 62, 61]);
  const [executions, setExecutions] = useState(0);
  const [successRate, setSuccessRate] = useState(98);
  const [avgLatency, setAvgLatency] = useState(234);
  const [channelStatuses, setChannelStatuses] = useState<{name: string; status: 'online' | 'warning' | 'offline'}[]>([
    { name: 'Aurora Monitor', status: 'offline' },
    { name: 'Telegram Bot', status: 'warning' },
    { name: 'Claude API', status: 'online' },
    { name: 'Ollama Gateway', status: 'online' },
  ]);
  ...
  useEffect(() => {
    ...
    aurora.onConnect(() => {
      ...
      setChannelStatuses(prev => prev.map(ch =>
        ch.name === 'Aurora Monitor' ? { ...ch, status: 'online' } : ch
      ));
    });
    ...
    aurora.onDisconnect(() => {
      setConnected(false);
      setChannelStatuses(prev => prev.map(ch =>
        ch.name === 'Aurora Monitor' ? { ...ch, status: 'offline' } : ch
      ));
    });

    aurora.onMetrics((newMetrics: AuroraMetrics) => {
      setMetrics(newMetrics);

      // Update CPU history
      setCpuHistory(prev => [...prev.slice(-12), newMetrics.cpu.percent]);

      // Update memory history
      setMemHistory(prev => [...prev.slice(-12), newMetrics.memory.percent]);

      // Simulate execution count increment
      setExecutions(prev => prev + Math.floor(Math.random() * 5));

      // Update channel statuses based on circuit breakers
      if (newMetrics.channels) {
        setChannelStatuses(prev => {
          const updated = [...prev];
          newMetrics.channels?.forEach(channel => {
            const existing = updated.find(ch => ch.name === channel.name);
            if (existing) {
              existing.status = channel.circuitState === 'closed' ? 'online' :
                               channel.circuitState === 'half-open' ? 'warning' : 'offline';
            } else {
              updated.push({
                name: channel.name,
                status: channel.circuitState === 'closed' ? 'online' :
                       channel.circuitState === 'half-open' ? 'warning' : 'offline'
              });
            }
          });
  ...
  const cpu = metrics?.cpu.percent ?? 45;
  const memory = metrics?.memory.percent ?? 62;
  const disk = metrics?.disk.percent ?? 71;
      ...
      <div className="grid grid-cols-12 gap-6 auto-rows-fr">
        ...
        <PremiumCard className="col-span-3" delay={0.1} color="cyan">
          <div className="flex flex-col h-full">
            <div className="flex items-center gap-3 mb-5">
              ...
              <span className="text-xs text-white/50 uppercase tracking-[0.2em] font-medium">Execucoes</span>
            </div>
            <div className="text-5xl font-extralight text-white mb-3">
              <MetricValue value={executions || 1847} color="#22d3ee" />
            </div>
        ...
        <PremiumCard className="col-span-3" delay={0.15} color="emerald">
          <div className="flex flex-col h-full">
            ...
            <div className="text-5xl font-extralight text-white mb-3">
              <MetricValue value={successRate} suffix="%" color="#10b981" />
            </div>
            <div className="flex items-center gap-2 text-emerald-400 text-sm mb-auto">
              <TrendingUp className="w-4 h-4" />
        ...
        <PremiumCard className="col-span-3" delay={0.2} color="purple">
          <div className="flex flex-col h-full">
            <div className="flex items-center gap-3 mb-5">
              ...
              <span className="text-xs text-white/50 uppercase tracking-[0.2em] font-medium">Tempo Medio</span>
            </div>
            <div className="text-5xl font-extralight text-white mb-3">
              <MetricValue value={avgLatency} suffix="ms" color="#8b5cf6" />
            </div>
            <div className="flex items-center gap-2 text-emerald-400 text-sm mb-auto">
              <TrendingUp className="w-4 h-4" />
            ...
            <div className="mt-4 pt-4 border-t border-white/5">
              <Sparkline data={[300, 280, 290, 260, 250, 240, 245, 235, 238, 234, 230, 225, avgLatency]} color="#8b5cf6" />
            </div>
          </div>
        </PremiumCard>
        ...
        <PremiumCard className="col-span-3" delay={0.25} color="amber">
          <div className="flex flex-col h-full">
            <div className="flex items-center gap-3 mb-5">
              ...
              <span className="text-xs text-white/50 uppercase tracking-[0.2em] font-medium">Protecoes Ativas</span>
            </div>
            <div className="text-5xl font-extralight text-white mb-3">
              <MetricValue value={channelStatuses.filter(c => c.status === 'online').length} color="#f59e0b" />
            </div>
            <div className="flex items-center gap-2 text-white/40 text-sm mb-auto">
              <span>de {channelStatuses.length} servicos</span>
            </div>
            <div className="mt-4 pt-4 border-t border-white/5">
              <Sparkline data={[40, 45, 50, 52, 55, 58, 60, 62, 64, 65, 66, 67, channelStatuses.length]} color="#f59e0b" />
            </div>
          </div>
        </PremiumCard>
        ...
        <PremiumCard className="col-span-4" delay={0.3} color="cyan">
          <div className="flex flex-col h-full">
            ...
            <div className="space-y-5 flex-1">
              <GlowingProgress value={Math.round(cpu)} color="#22d3ee" label="CPU" />
              <GlowingProgress value={Math.round(memory)} color="#8b5cf6" label="Memoria" />
              <GlowingProgress value={28} color="#10b981" label="Rede" />
              <GlowingProgress value={Math.round(disk)} color="#f59e0b" label="Disco" />
            </div>
          </div>
        </PremiumCard>
        ...
        <PremiumCard className="col-span-4" delay={0.35} color="purple">
          <div className="flex flex-col h-full">
            ...
            <div className="space-y-4 flex-1">
              {channelStatuses.map((channel, i) => (
                <StatusPulse key={i} status={channel.status} label={channel.name} />
              ))}
            </div>
          </div>
        </PremiumCard>
        ...
        <PremiumCard className="col-span-4" delay={0.4} color="pink">
          <div className="flex flex-col h-full">
            ...
            <div className="space-y-3 text-sm flex-1">
              {[
                { time: '2s', text: 'Aurora Monitor iniciado', color: '#22d3ee' },
                { time: '5s', text: 'Circuit Breaker ativo', color: '#10b981' },
                { time: '10s', text: 'Rate Limiter configurado', color: '#8b5cf6' },
                { time: '15s', text: 'Metricas coletadas', color: '#f59e0b' },
                { time: '20s', text: 'Auto-healing pronto', color: '#ec4899' },
              ].map((item, i) => (
                <motion.div
                  key={i}
