...
// P1 Skills
import { TestRunSkill, TestRunOutput } from '../../skills/testing/run';
import { AnalyzeErrorSkill, AnalyzeErrorOutput, ErrorInfo } from '../../skills/analyze/error';
import { PatchApplySkill, PatchOperation } from '../../skills/patch/apply';
...
export class HubEnterpriseOrchestrator extends Skill {
  ...
  // P1 Skills
  private testRun = new TestRunSkill();
  private analyzeError = new AnalyzeErrorSkill();
  private patchApply = new PatchApplySkill();
  ...
  private async workflowCodeOnly(
    ...
    // Step 5: P1 - Correction loop if build failed
    let correctionResult: { success: boolean; attempts: number; backups: string[] } | undefined;
    if (execStep.status === 'failed') {
      this.logger.info('P1: Build failed, entering correction loop');
      correctionResult = await this.runCorrectionLoop(
        appLocation,
        executionId,
        steps,
        errors,
      ...
      if (correctionResult.success) {
        this.logger.info(`P1: Build fixed after ${correctionResult.attempts} attempts`);
      } else {
        this.logger.warn(`P1: Could not fix build after ${correctionResult.attempts} attempts`);
      }
    }

    // Step 6 (or later if P1 ran): P0 - Coletar artefatos
  ...
  /**
   * P1 CORRECTION LOOP
   * Tenta corrigir erros de build/typecheck automaticamente
  ...
  ): Promise<{ success: boolean; attempts: number; backups: string[] }> {
    const MAX_ATTEMPTS = 2;
    let attempts = 0;
    const backups: string[] = [];

    this.logger.info('P1: Starting correction loop');

    while (attempts < MAX_ATTEMPTS) {
      attempts++;
      this.logger.info(`P1: Correction attempt ${attempts}/${MAX_ATTEMPTS}`);

      // Step 1: Run typecheck
      const testStep: WorkflowStep = {
        persona: 'p1-corrector',
        subskill: 'test.run',
        status: 'in-progress',
        startTime: Date.now(),
      };
      ...
      const testResult = await this.testRun.run({
        params: {
          appLocation,
          command: 'typecheck',
          executionId: `${executionId}-attempt-${attempts}`,
        },
      });

      testStep.duration = Date.now() - testStep.startTime!;

      if (testResult.success) {
        // Typecheck passed!
        testStep.status = 'success';
        testStep.result = testResult.data;
        this.logger.info('P1: Build passed, correction loop complete');
        return { success: true, attempts, backups };
      }

      // Typecheck failed - analyze error
      testStep.status = 'failed';
      testStep.error = testResult.error;

      const testOutput = testResult.data as TestRunOutput | undefined;
      if (!testOutput?.errorAnalysis) {
        this.logger.warn('P1: No error analysis available');
        errors.push({ error: 'No error analysis available' });
        continue;
      }

      commandsRun.push({
        cmd: testOutput.command,
        args: testOutput.args,
        status: 'failed',
        exitCode: testOutput.exitCode,
        stdout: testOutput.stdout,
        stderr: testOutput.stderr,
        duration: testOutput.duration,
        logFile: testOutput.logFile,
      });

      // Step 2: Analyze error
      const analyzeStep: WorkflowStep = {
        persona: 'p1-corrector',
        subskill: 'analyze.error',
        status: 'in-progress',
        startTime: Date.now(),
      };
      steps.push(analyzeStep);

      const errorAnalysis = testOutput.errorAnalysis;
      const primaryError = errorAnalysis.primaryError;

      if (!primaryError || !primaryError.file) {
        analyzeStep.status = 'failed';
        analyzeStep.error = 'No file-specific error found';
        analyzeStep.duration = Date.now() - analyzeStep.startTime!;
        this.logger.warn('P1: Cannot auto-fix - no file identified');
        errors.push({ error: 'No file-specific error found for auto-fix' });
        continue;
      }

      analyzeStep.status = 'success';
      analyzeStep.result = errorAnalysis;
      analyzeStep.duration = Date.now() - analyzeStep.startTime!;
      this.logger.info(`P1: Primary error: ${primaryError.errorCode} in ${primaryError.file}:${primaryError.line}`);

      // Step 3: Generate and apply patch
      const patchStep: WorkflowStep = {
        persona: 'p1-corrector',
        subskill: 'patch.apply',
        status: 'in-progress',
        startTime: Date.now(),
      };
      ...
      const patch = this.generatePatchForError(primaryError);
      if (!patch) {
        patchStep.status = 'failed';
        patchStep.error = `Cannot generate patch for error: ${primaryError.errorCode}`;
        patchStep.duration = Date.now() - patchStep.startTime!;
        this.logger.warn('P1: Cannot generate patch for this error type');
        errors.push({ path: primaryError.file, error: `Cannot auto-fix: ${primaryError.errorCode}` });
        continue;
      }

      const patchResult = await this.patchApply.run({
        params: {
          appLocation,
          patches: [patch],
          createBackup: true,
      ...
      if (patchResult.success) {
        patchStep.status = 'success';
        patchStep.result = patchResult.data;
        const backupDir = patchResult.data?.backupDir;
        if (backupDir) backups.push(backupDir);
        this.logger.info(`P1: Patch applied to ${patch.file}`);
      } else {
        patchStep.status = 'failed';
        patchStep.error = patchResult.error;
        errors.push({ path: patch.file, error: patchResult.error || 'Patch failed' });
        this.logger.warn(`P1: Patch failed: ${patchResult.error}`);
        continue;
      }
    }

    this.logger.warn(`P1: Correction loop exhausted after ${attempts} attempts`);
    return { success: false, attempts, backups };
  }

  /**
   * Gera patch baseado no tipo de erro
   */
  private generatePatchForError(error: ErrorInfo): PatchOperation | null {
    if (!error.file) return null;

    // TS7006: Parameter has implicit any
    if (error.errorCode === 'TS7006') {
      // Simplest fix: add `: any` to the parameter
      // This requires the actual file content to do properly
      // For now, return null as this needs more context
      return null;
    }

    // TS2304: Cannot find name
    if (error.errorCode === 'TS2304' && error.message.includes("Cannot find name")) {
      // Extract the missing identifier
      const match = error.message.match(/Cannot find name '([^']+)'/);
      if (match) {
        const identifier = match[1];
        // If it looks like a type, we could add a type declaration
        // For now, we'll add a simple declare statement at the top
        return {
          file: error.file,
          operation: 'insert',
          line: 1,
          replacement: `declare const ${identifier}: any; // Auto-fix by P1 corrector\n`,
        };
      }
    }

    // TS2345: Argument type mismatch - common with string vs number
    if (error.errorCode === 'TS2345') {
      // This is too context-specific for auto-fix
      return null;
    }

    // TS2339: Property does not exist - could add type assertion
    if (error.errorCode === 'TS2339') {
